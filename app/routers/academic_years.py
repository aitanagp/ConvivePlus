...
async def create_year(year_in: AcademicYearIn, current_user: UserDb = Depends(get_current_admin)):
    # Solo el Superusuario o el Director pueden hacer esto según RF14
    if current_user.role != 'ROOT' and current_user.role != 'DIRECTOR':
        raise HTTPException(status_code=403, detail="Solo el Superusuario o el Director pueden gestionar cursos académicos")
...
async def set_current(year_id: int, current_user: UserDb = Depends(get_current_admin)):
    if current_user.role != 'ROOT' and current_user.role != 'DIRECTOR':
        raise HTTPException(status_code=403, detail="Solo el Superusuario o el Director pueden cambiar el curso activo")
